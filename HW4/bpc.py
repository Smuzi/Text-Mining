...
def kmean(X,k,criteria,initial_centroids=None):
	...
	#endadd
	return kmean_body(X,closest_cluster_id,criteria)
	
	

def kmean_body(X,closest_cluster,criteria):
	counter = 0
...
def main(wc,dc,filename,docOutput,wordOutput,wordDF,criteria):
	metric = 'cosine'
	num_WC = wc
	num_DC = dc
	...
	Word2WordC,Doc2DocC = bipartite_clustering(Word2Doc,num_WC,num_DC,metric,criteria)
	#Doc2DocC,Word2WordC = bipartite_clustering(Doc2Word,num_DC,num_WC,metric,criteria)
